...
def train_www(args, model, device, loader, optimizers, task_type, optimizer_name,loss_logger,cluster_ids,sec_cluster_ids,cluster_centers):
    ...
    model.train()
    # if optimizer_name == 'predictor':
    #     set_requires_grad([model.graph_encoder, model.predictor,model.env_mlp], requires_grad=True)
    #     set_requires_grad([model.separator], requires_grad=False)
    # if optimizer_name == 'separator':
    #     set_requires_grad([model.separator], requires_grad=True)
    #     set_requires_grad([model.graph_encoder,model.predictor,model.env_mlp], requires_grad=False)
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            # target_rep = batch.y.to(torch.float32).repeat_interleave(batch.batch[-1]+1,dim=0)
            # is_labeled_rep = target_rep == target_rep
            loss += criterion(pred['pred_gnn'].to(torch.float32)[is_labeled], target[is_labeled])
            
            loss += args.pred_augument_gnn * criterion(pred['pred_augument_gnn'].to(torch.float32)[is_labeled], target[is_labeled])
