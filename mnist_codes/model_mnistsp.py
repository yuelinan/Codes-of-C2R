...
class Graph_C2R(torch.nn.Module):
    ...
    def __init__(self, args, num_tasks, num_layer = 5, emb_dim = 300, gnn_type = 'gin', drop_ratio = 0.5, gamma = 0.4, use_linear_predictor=False):
        ...
        self.node_enoder = nn.Linear(5,self.emb_dim)
    ...
    def forward(self, batched_data,ori_cluster_one_hot=None,cluster_one_hot=None,cluster_centers=None):
        ...
        if cluster_one_hot  == None:
            ...
            batched_data.edge_attr = batched_data.edge_attr.unsqueeze(-1)
            ...
            output = {"pred_rep":pred_rep,'pred_gnn': pred_gnn, 'pred_rem': pred_rem, 'loss_reg':loss_reg,'pred_augument_gnn':pred_augument_gnn, 'cycle_loss':cycle_loss, 'teacher_loss':teacher_loss}
            return output
