...
## training
from model import Graph_C2R
from utils import init_weights, get_args, eval_spmotif,train_www_epoch0,eval_spmotif_explain,train_www,get_kmeans
import json
...
def main(args,logger):
    ...
    model = Graph_WWW_Gumbel( gnn_type = args.gnn, num_tasks = 3, num_layer = args.num_layer,
    ...
    opt_predictor = optim.Adam(list(model.graph_encoder.parameters())+list(model.predictor.parameters())+ list(model.node_enoder.parameters()) +list(model.infonce.parameters()) +list(model.infonce2.parameters()) + list(model.env_mlp.parameters()), lr=args.lr, weight_decay=args.l2reg)
    ...
    for epoch in range(args.epochs):
        ...
        if update_test or epoch == 0:
            ...
            json_str = json.dumps({'Train': train_perf, 'Validation': valid_perf, 'Test': test_perfs[0] }, ensure_ascii=False)
            ...
            f_json.write(json_str)
            f_json.write('\n')
            PATH = '/data/lnyue/GREA-main/spmotif_codes/www_output/'+'_www_gin_0.9_'+str(epoch)
